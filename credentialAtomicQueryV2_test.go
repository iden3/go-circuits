...
func TestAtomicQuerySigV2_PrepareInputs(t *testing.T) {
	...
	var stateInOnChainSmt StateInOnChainSmt
	stateInOnChainSmt.Proof, _, err = onChainSmt.GenerateProof(ctx, userIdentity.BigInt(), nil)
	assert.Nil(t, err)

	stateInOnChainSmt.Root = onChainSmt.Root()
	...
	atomicInputs := AtomicQuerySigV2Inputs{
...
func TestAtomicQuerySigV2Outputs_CircuitUnmarshal(t *testing.T) {
	userStateInOnChainSmtRootInt, ok := new(big.Int).SetString("2960269998131412406135915396987536312795307713692807443361231572350088373156", 10)
	assert.True(t, ok)
	userStateInOnChainSmtRoot, err := merkletree.NewHashFromBigInt(userStateInOnChainSmtRootInt)
	assert.NoError(t, err)

	userNullifier, _ := new(big.Int).SetString("987654321", 10)
	...
	expectedOut := AtomicQuerySigV2PubSignals{
		UserStateInOnChainSmtRoot: userStateInOnChainSmtRoot,
		UserID:                    userNullifier,
