...
func TestAtomicQuerySigV2_PrepareInputs(t *testing.T) {
	...
	assert.Nil(t, err)
	stateInOnChainSmtProof, err := onChainSmt.GenerateCircomVerifierProof(ctx, userIdentity.BigInt(), nil)
	assert.Nil(t, err)
	...
	atomicInputs := AtomicQuerySigV2Inputs{
		StateInOnChainSmtProof: stateInOnChainSmtProof,
		Salt:                   salt,
