...
func TestAttrQuerySigOnChainSmt_PrepareInputs(t *testing.T) {
	...
	nullifier, err := poseidon.Hash([]*big.Int{salt, userPrivateKey.Public().X, userPrivateKey.Public().Y})
	assert.Nil(t, err)
